...
*Key constraints:* Only the /play endpoint is exposed on this port to minimize the attack surface while meeting tournament requirements.
...
| Automated CD Pipeline: GitHub Actions handles the SSH connection, git pull, and docker compose restart automatically upon a new Release.
