...
* **Uniqueness conflicts (Registration and creation):** The Gatling script triggers simulations using static (hardcoded) data. By simulating 50 concurrent users attempting to register the same email address or username in the same millisecond, the backend processes the first one and correctly rejects the remaining 49 to avoid data duplication in the database.
