...
use crate::{
    Coordinates, GameAction, GroupExpansionBot, Movement, RandomBot, RenderOptions, YBot, YBotRegistry, game
};
...
pub fn run_cli_game() -> Result<()> {
    ...
    let bots_registry = YBotRegistry::new().with_bot(Arc::new(RandomBot)).with_bot(Arc::new(GroupExpansionBot));
