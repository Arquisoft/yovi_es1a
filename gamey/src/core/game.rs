...
impl GameY {
    ...
    pub fn cells_for_player(&self, target_player: PlayerId) -> Vec<Coordinates> {
        ...
        // Iteramos sobre el HashMap 'board_map' que guarda el estado de cada celda
        for (coords, (_, player)) in &self.board_map {
            // Si el PlayerId de esa celda coincide con el que buscamos, guardamos la coordenada
